# middlemanブランチのpush時にのみ作動する想定
MASTER_REPOSITORY = if ENV['GH_TOKEN']
    'https://$GH_TOKEN@github.com/3100/3100.github.io'
  else
    'git@github.com:3100/3100.github.io.git'
  end
...
def push_to_gh_pages(repository, branch)
  ...
  Dir.chdir DEST_DIR do
    sh 'git add -A'
    sh "git commit -m 'Update with #{sha1} [ci skip]'"
